...
class AppController:
            ...
            for control in self.sidebar_column.controls:
                if isinstance(control, ft.Container):
                    ...
                    if isinstance(control.content, ft.Row) and len(control.content.controls) > 1:
                        control.content.controls[1].color = "#f9fafb" if is_active else "#9ca3af"
                    control.update()
    ...
    def update_content_view(self, target, should_update=True):
        print(f"[AppController.update_content_view] Target: {target}, Should Update: {should_update}")
    ...
    def trigger_emergency(self):
        ...
        self.page.banner = ft.Banner(
            ...
            content=ft.Text("ALERTA DE EVACUACIÓN ENVIADA - MODO EMERGENCIA", color=ft.Colors.WHITE),
            actions=[
                ft.TextButton("Desactivar", style=ft.ButtonStyle(color=ft.Colors.WHITE), on_click=self.close_banner)]
        )
