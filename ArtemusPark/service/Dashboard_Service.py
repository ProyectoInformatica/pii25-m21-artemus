...
class DashboardService:
    ...
    def get_temp_chart_data(self) -> List[Dict[str, Any]]:
        ...
        for i, item in enumerate(recent):
            ts = item.get("timestamp", 0) if isinstance(item, dict) else getattr(item, "timestamp", 0)
    ...
    def get_sensors_health_status(self) -> List[Dict[str, Any]]:
        ...
        def check_status(data_list, name_sensor, icon):
            ...
            if data_list:
                ...
                ts = last_item.get("timestamp", 0) if isinstance(last_item, dict) else getattr(last_item, "timestamp",
                                                                                               0)
                if (now - ts) < threshold: is_online = True
                try:
        ...
        health_report = [
            check_status(Temperature_Repository.load_all_temperature_measurements(), "Sensor Temperatura",
                         ft.Icons.THERMOSTAT),
            check_status(Humidity_Repository.load_all_humidity_measurements(), "Sensor Humedad", ft.Icons.WATER_DROP),
            check_status(Wind_Repository.load_all_wind_measurements(), "Anemómetro", ft.Icons.AIR),
            check_status(Smoke_Repository.load_all_smoke_measurements(), "Detector Humo", ft.Icons.CLOUD),
            check_status(Door_Repository.load_all_door_events(), "Puertas", ft.Icons.SENSOR_DOOR),
            check_status(Light_Repository.load_all_light_events(), "Iluminación", ft.Icons.LIGHTBULB),
        ]
    ...
    def _get_last_value(self, data_list: List[Dict], key: str, default: Any):
        ...
        last = data_list[-1]
        if isinstance(last, dict): return last.get(key, default)
