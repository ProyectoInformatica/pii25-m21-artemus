...
class MapCard(ft.Container):
    def __init__(self, on_sensor_click=None):
        ...
        self.sensor_config = [
            ...
            ("temperature", 0.375, 0.72, AppColors.DANGER, "Temperatura", ft.Icons.THERMOSTAT),
            ...
            ("smoke", 0.375, 0.28, ft.Colors.ORANGE_700, "Calidad Aire", ft.Icons.CLOUD),
        ]
    ...
    def update_light_marker_status(self, is_on: bool, consumption: float):
        ...
        if marker:
            marker_icon_control = marker.content  # Esto es el ft.Icon dentro del Container
            marker_icon_control.icon = ft.Icons.LIGHTBULB if is_on else ft.Icons.LIGHTBULB_OUTLINE
    ...
    def update_sensor_data(self, data: dict):
        ...
        if "light_is_on" in data and "light_consumption" in data:
            self.update_light_marker_status(data["light_is_on"], data["light_consumption"])
