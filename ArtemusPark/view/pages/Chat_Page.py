...
class ChatPage(ft.Container):
    def __init__(self, current_username, current_user_role, permissions=None):
        ...
        self.btn_edit_name = ft.IconButton(ft.Icons.EDIT, tooltip="Editar nombre del chat", on_click=self._handle_edit_chat_name, visible=False)
        self.btn_manage_members = ft.IconButton(ft.Icons.PEOPLE_OUTLINE, tooltip="Gestionar miembros", on_click=self._handle_manage_members, visible=False)
        self.btn_delete_chat = ft.IconButton(ft.Icons.DELETE_OUTLINE, tooltip="Borrar chat", icon_color="red", on_click=self._handle_delete_chat, visible=False)
                ...
                ft.Container(
                    content=ft.Row([
                        ...
                        ft.Container(
                            ...
                            bgcolor="red", width=20, height=20, border_radius=10,
                            alignment=ft.alignment.center, visible=unread_count > 0 and not is_selected
                        )
                    ]),
                    padding=10, border_radius=10,
                    bgcolor=AppColors.ACCENT if is_selected else "white",
                    on_click=lambda e, cid=chat["id_chat"], cname=chat["name"], ig=is_group: self._select_chat(cid, cname, ig),
                    ink=True
                )
            )
        try: self.update()
    ...
    def _handle_search(self, e):
        ...
        if not self.selected_chat_id: return
    ...
    def _display_messages(self, filter_query=""):
        ...
        for msg in self.messages:
            ...
            bg_color = ft.Colors.RED_100 if is_alert else (AppColors.ACCENT_SOFT if is_me else "white")
            
            if msg.get("sender_dni") == "12345678X":
                 sender_name = "Bot-Artemus"
            else:
                 sender_name = msg["full_name"] or msg["username"]
                 if is_me: sender_name = f"{sender_name} (Tú)"
            
            self.messages_column.controls.append(
                ft.Row([
                    ft.Container(
                        content=ft.Column([
                            ft.Text(sender_name, size=10, weight=ft.FontWeight.BOLD, color=ft.Colors.RED_700 if is_alert else AppColors.ACCENT),
                        ...
                        bgcolor=bg_color,
                        border=ft.border.all(2, ft.Colors.RED_700) if is_alert else None,
    ...
    def _handle_send_click(self, e):
        ...
        if "SEND_MESSAGES" not in self.permissions:
            self.page.open(ft.SnackBar(content=ft.Text("No tienes permiso para enviar mensajes")))
            return

        text = self.message_input.value.strip()
        if not self.selected_chat_id or not text: return
    ...
    def _handle_share_status(self, e):
        if "SEND_MESSAGES" not in self.permissions: return
        if not self.selected_chat_id: return
        data = self.dashboard_service.get_latest_sensor_data()
        msg = (
    ...
    def _handle_manage_members(self, e):
        ...
        def toggle_member(dni, is_adding):
            if is_adding: self.chat_repo.add_user_to_chat(self.selected_chat_id, dni)
            else: self.chat_repo.remove_user_from_chat(self.selected_chat_id, dni)
            self.page.pubsub.send_all("new_chat_message")

        for uname, udata in all_users.items():
            if udata["dni"] == self.user_dni: continue
            is_in = udata["dni"] in participant_dnis
            user_list_items.controls.append(
                ft.Checkbox(
                    label=f"{udata['full_name'] or uname} (@{uname})",
                    value=is_in,
                    on_change=lambda e, udni=udata["dni"]: toggle_member(udni, e.control.value)
                )
            )
    ...
    def _handle_edit_chat_name(self, e):
        if not self.selected_chat_id: return
        name_input = ft.TextField(label="Nuevo nombre", value=self.chat_header_text.value)
        def save_name(e):
    ...
    def _handle_delete_chat(self, e):
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Text("¿Estás seguro de que quieres borrar este chat y todos sus mensajes?"),
            actions=[
                ft.TextButton("Cancelar", on_click=lambda e: self.page.close(dialog)),
    ...
    def _build_ui(self):
        return ft.Row(
            ...
            controls=[
                ft.Container(
                    width=300, bgcolor="white", border_radius=12, padding=15,
                    content=ft.Column([
                        ft.Text("Mensajes", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.BLACK),
                        ft.Divider(),
                        self.chat_list_column,
                        ft.ElevatedButton("Nuevo Chat / Grupo", icon=ft.Icons.ADD, on_click=self._show_new_chat_dialog)
                    ])
                ),
                ft.Container(
                    expand=True, bgcolor=AppColors.GLASS_WHITE, border_radius=12, padding=20,
                    content=ft.Column([
                        ft.Row([
                            ft.Column([
                        ...
                        ft.Divider(),
                        ft.Container(expand=True, content=self.messages_column, padding=ft.padding.only(bottom=10)),
                        ft.Row([
                            ft.IconButton(ft.Icons.BAR_CHART, tooltip="Compartir estado del parque", on_click=self._handle_share_status),
                            self.message_input,
                            ft.IconButton(icon=ft.Icons.SEND_ROUNDED, icon_color=AppColors.ACCENT, on_click=self._handle_send_click)
                        ])
                    ])
                )
            ]
    ...
    def _show_new_chat_dialog(self, e):
        ...
        chat_name_input = ft.TextField(label="Nombre del grupo (opcional)", hint_text="Solo para grupos")
        def toggle_user(dni, checkbox):
            if checkbox.value: selected_users.add(dni)
            elif dni in selected_users: selected_users.remove(dni)
        for user in users:
            cb = ft.Checkbox(label=f"{user['full_name'] or user['username']} (@{user['username']})", on_change=lambda e, udni=user["dni"]: toggle_user(udni, e.control))
            user_list.controls.append(cb)
        def create_group(e):
            if len(selected_users) < 2: return
            name = chat_name_input.value.strip() or (f"Grupo de {len(selected_users)} personas" if len(selected_users) > 2 else "Chat")
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Column([chat_name_input, ft.Text("Selecciona participantes:"), user_list], tight=True, width=400),
            actions=[ft.TextButton("Cancelar", on_click=lambda e: self.page.close(dialog)), ft.ElevatedButton("Crear", on_click=create_group)]
        )
        self.page.open(dialog)
