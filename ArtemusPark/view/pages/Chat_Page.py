...
class ChatPage(ft.Container):
                    ...
                    [
                        ft.Container(
                            content=ft.Column(
                                [
                                    ft.Text(
                                        msg["username"] if not is_me else "Tú",
                                        size=10,
                                        weight=ft.FontWeight.BOLD,
    ...
    def _handle_send_click(self, e):
        ...
        self.chat_repo.send_message(
            self.selected_chat_id, self.user_dni, self.message_input.value
        )
    ...
    def _show_new_chat_dialog(self, e):
        ...
        def start_chat(target_dni, target_name):
            chat_id = self.chat_repo.create_chat(
                f"Chat con {target_name}", [self.user_dni, target_dni]
            )
        ...
        for user in users:
            user_list.controls.append(
                ft.ListTile(
                    ...
                    on_click=lambda e, udni=user["dni"], uname=user[
                        "username"
                    ]: start_chat(udni, uname),
                )
            )
