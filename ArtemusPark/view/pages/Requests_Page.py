...
class RequestsPage(ft.Container):
    ...
    def _build_request_card(self, req):
        ...
        card_content = [
            ft.Row([
                ...
                ft.Text(datetime.fromtimestamp(req.get("timestamp", 0)).strftime("%Y-%m-%d %H:%M"), size=12, color=ft.Colors.GREY)
            ]),
            ft.Text(f"Solicitante: {req.get('user')}", weight="bold", color=ft.Colors.BLACK87),
            ...
            card_content.append(
                ft.Row([
                    ft.ElevatedButton(
                        ...
                        on_click=lambda e, r=req: self._handle_request(r, "ACCEPTED")
                    ),
                    ft.ElevatedButton(
                        "Rechazar", 
                        icon=ft.Icons.CLOSE, 
                        ...
                        color=ft.Colors.WHITE,
                        on_click=lambda e, r=req: self._handle_request(r, "REJECTED")
                    ),
    ...
    def _handle_request(self, req, new_status):
        ...
        self.page.snack_bar = ft.SnackBar(ft.Text(f"Solicitud marcada como {new_status}"))
