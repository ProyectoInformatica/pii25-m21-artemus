        ...
        if data:
            ...
            if profile_pic:
                ...
                self.user_avatar.content = ft.Image(
                    src_base64=b64, border_radius=50, fit=ft.ImageFit.COVER
                )
    ...
    def _on_file_result(self, e: ft.FilePickerResultEvent):
        if e.files:
            ...
            self.page.pubsub.send_all(
                {"topic": "profile_updated", "username": self.username}
            )
            self._load_data()
            self.page.open(
                ft.SnackBar(ft.Text("Foto de perfil actualizada"), bgcolor="green")
            )

    def _handle_save(self, e):
        if (
            not self.tf_full_name.value
            or not self.tf_dni.value
            or not self.tf_phone.value
        ):
            self.page.open(
                ft.SnackBar(
                    ft.Text("Nombre, DNI y Teléfono son obligatorios"), bgcolor="red"
                )
            )
            return
        ...
        self.page.open(
            ft.SnackBar(ft.Text("Perfil actualizado correctamente"), bgcolor="green")
        )
